...
void HID_API_EXPORT
HID_API_CALL hid_free_enumeration(struct hid_device_info *devs) {
...
HID_API_EXPORT hid_device
*
HID_API_CALL hid_open_path(const char *path) {
    hid_device * dev;
    HIDP_CAPS caps;
    PHIDP_PREPARSED_DATA pp_data = NULL;
    BOOLEAN res;
...
#include <stdint.h>
static char *dumpHex(const char *data, size_t size) {
    char *buf = malloc(sizeof(char) * 255);

    buf[0] = '[';
    size_t lastPos = 1;
    for (size_t i = 0; i < size; i++) {
        sprintf(buf + lastPos, "%02x ", (uint8_t) data[i]);
        lastPos += 3;
    }
    buf[lastPos] = ']';
    buf[lastPos + 1] = '\0';
...
int HID_API_EXPORT
HID_API_CALL hid_write(hid_device *dev, const unsigned char *data, size_t length) {
    DWORD bytes_written;
    BOOL res;

    OVERLAPPED s_overlapped;
    unsigned char *buf;
    memset(&s_overlapped, 0, sizeof(s_overlapped));
...
int HID_API_EXPORT
HID_API_CALL hid_read_timeout(hid_device *dev, unsigned char *data, size_t length, int milliseconds) {
    DWORD bytes_read = 0;
    size_t copy_len = 0;
    BOOL res;
...
//            fprintf(stdout, "Strip first response byte, because DDK came from 90's\n");
...
int HID_API_EXPORT
HID_API_CALL hid_read(hid_device *dev, unsigned char *data, size_t length) {
    return hid_read_timeout(dev, data, length, INFINITE);
}

int HID_API_EXPORT
HID_API_CALL hid_set_nonblocking(hid_device *dev, int nonblock) {
    dev->blocking = !nonblock;
    return 0; /* Success */
}

int HID_API_EXPORT
HID_API_CALL hid_send_feature_report(hid_device *dev, const unsigned char *data, size_t length) {
...
int HID_API_EXPORT
HID_API_CALL hid_get_feature_report(hid_device *dev, unsigned char *data, size_t length) {
    BOOL res;
#if 0
...
int HID_API_EXPORT
HID_API_CALL hid_get_input_report(hid_device *dev, unsigned char *data, size_t length) {
#if 0
    BOOL res;
...
void HID_API_EXPORT
HID_API_CALL hid_close(hid_device *dev) {
    if (!dev)
        return;
    CancelIo(dev->device_handle);
    free_hid_device(dev);
...
int HID_API_EXPORT_CALL
HID_API_CALL hid_get_product_string(hid_device *dev, wchar_t *string, size_t maxlen) {
    BOOL res;
...
int HID_API_EXPORT_CALL
HID_API_CALL hid_get_serial_number_string(hid_device *dev, wchar_t *string, size_t maxlen) {
    BOOL res;
...
int HID_API_EXPORT_CALL
HID_API_CALL hid_get_indexed_string(hid_device *dev, int string_index, wchar_t *string, size_t maxlen) {
    BOOL res;
...
*
HID_API_CALL hid_error(hid_device *dev) {
    if (dev) {

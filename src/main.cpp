...
static void action_sign_tx_opts(boost::program_options::options_description &options) {
    options.add_options()
               ("hash", po::value<std::string>()->required(), "Incomplete transaction sha3 hash (64 byte hex string)")
               ("index,i", po::value<uint32_t>()->default_value(0), "BIP32 derivation index")
               ("inline", "Print result inline without newlines etc");
}
...
Options)";
    ...
    desc.add_options()
            ("help,h", "Prints this help or use it with command name to see additional parameters")
            ("version,v", "print version of client")
            ("verbose", "print data transfer process");

...
if ((opts_cnt == 0)); then
  # iterate only over available c++ stdlib
  for stdlib_idx in ${!std_lib_names[*]}; do
    CONAN_LOCAL=1 conan create . ${CONAN_REPO}/${CONAN_REPO_TAG} -s compiler.libcxx="${std_lib_names[$stdlib_idx]}" -s build_type=Debug --build=missing
    CONAN_LOCAL=1 conan create . ${CONAN_REPO}/${CONAN_REPO_TAG} -s compiler.libcxx="${std_lib_names[$stdlib_idx]}" -s build_type=Release --build=missing
  done
fi
